...
// Wed Aug 26 23:00:49 2026 by ROOT version 6.34.08
// from TTree Events/Events
// found on file: 0082C29D-E74C-024A-BE9B-97B29EE7A4A2.root
//////////////////////////////////////////////////////////

#ifndef minhaAnalise_h
#define minhaAnalise_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Float_t         Jet_btagDeepCvB[21];   //[nJet]
   Float_t         Jet_btagDeepCvL[21];   //[nJet]
   ...
   minhaAnalise(TTree *tree=0);
   virtual ~minhaAnalise();
...
#ifdef minhaAnalise_cxx
minhaAnalise::minhaAnalise(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("0082C29D-E74C-024A-BE9B-97B29EE7A4A2.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("0082C29D-E74C-024A-BE9B-97B29EE7A4A2.root");
      }
...
minhaAnalise::~minhaAnalise()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t minhaAnalise::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t minhaAnalise::LoadTree(Long64_t entry)
{
...
void minhaAnalise::Init(TTree *tree)
{
...
void minhaAnalise::Show(Long64_t entry)
{
...
Int_t minhaAnalise::Cut(Long64_t entry)
{
